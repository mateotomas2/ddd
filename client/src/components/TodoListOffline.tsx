...
import { Button, Checkbox, Divider, Grid, List, ListItem, Sheet, TextField } from '@mui/joy';
import { v4 } from 'uuid';
...
const reducer = (state, event) => {
  switch (event.constructor.name) {
    ...
    case "TodoRemovedEvent":
      ...
      break;
    case "TodoMarkedDone":
      state.todos = state.todos.map((todo) => todo.id == event.id ? { ...todo, done: true } : todo)
      break;
    case "TodoMarkedUndone":
      state.todos = state.todos.map((todo) => todo.id == event.id ? { ...todo, done: false } : todo)
      break;
  }
  return state;
}
...
export default function TodoListOffline() {
  ...
  return (
    ...
    >
      <DevTool events={events} onClickEvent={(index) => {
        recreateStateUntilEventIndex(index);
        setEventIndex(index);
      }} currentEventIndex={currentEventIndex} />
      <Box >
        <List variant="soft" sx={{ margin: "0px 0px 10px 0px", p: 0, borderRadius: 'sm' }} >
          <AnimatePresence>
            {todos.todos.map((todo, index) => (
              ...
              >
                <ListItem >
                  <Checkbox label={todo.text} overlay checked={todo.done} onChange={(ev) => {
                    if (ev.target.checked) dispatch(new TodoMarkedDone(
                      todo.id,
                    ))
                    else dispatch(new TodoMarkedUndone(
                      todo.id,
                    ))
                  }} />
                </ListItem>
